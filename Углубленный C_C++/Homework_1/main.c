...
int main (void)
{
    ...
    1) Вводимые пользователем строки могут содержать только буквы или знаки пробела. Клички дополнительно могут содержать цифры. \n\
    ...
    while (!stop_flag) { 
        ...
        char typ[MAXLEN];
        get_correct_string(console_in, console_out, PATTERN_ANIMAL, typ); // считаем вид животного
        show_by_group(console_out, mas, n, typ); // выведем всех представителей вида
